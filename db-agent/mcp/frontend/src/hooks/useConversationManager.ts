    ...
    if (conversation.threadId) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.ok) {
            // 转换后端消息格式：[{role, content}] -> [{message: {role, content}, status: 'success'}]
            ...
            console.log(`   原始消息数: ${backendMessages.length}`);
            console.log(`   转换后消息数: ${convertedMessages.length}`);
            if (backendMessages.length > 0) {
              console.log(`   第一条原始消息:`, backendMessages[0]);
              console.log(`   第一条转换后:`, convertedMessages[0]);
            }
